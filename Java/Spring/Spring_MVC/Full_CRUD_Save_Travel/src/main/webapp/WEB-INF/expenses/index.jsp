				...
				<tbody>
					<c:forEach var="expense" items="${expenses}">
						<tr>
							...
							<td><a href="/expenses/<c:out value="${expense.getId()}"/>"><c:out
										value="${expense.getName()}" /></a></td>
							<td><c:out value="${expense.getVendor()}" /></td>
							<td>$<c:out value="${expense.getAmount()}" /></td>
							<td>
								<div class="row">
									<a class="btn btn-primary col-3 offset-2"
										href="/expenses/edit/<c:out value="${expense.getId()}" />">Edit</a>

									<form class="col-6"
										action="/expenses/<c:out value="${expense.getId()}"/>"
										method="post">
